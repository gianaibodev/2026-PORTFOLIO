...
// Lazy load heavy Three.js-based Hero component (~37MB Three.js bundle)
...
export default function HomeClient({
  ...
  useEffect(() => {
    ...
    const HOME_INTRO_KEY = "homeIntroShown_v1";

    // Hard bypass for screenshot generation / share previews
    if (skipIntro || isLowEnd) {
      sessionStorage.setItem(HOME_INTRO_KEY, "true");
      setShowLoader(false);
      return;
    }

    const hasSeen = sessionStorage.getItem(HOME_INTRO_KEY);
    if (!hasSeen) {
      setShowLoader(true);
      sessionStorage.setItem(HOME_INTRO_KEY, "true");
  ...
  useEffect(() => {
    if (!showLoader && mounted) {
      const timeoutId = setTimeout(() => {
        setShowWarp(true);
        setTimeout(() => setShowWarp(false), 900);
      }, 0);
      ...
      <div className="w-full overflow-x-hidden">
        {showLoader && (
          <div className="fixed inset-0 z-[99999]">
            ...
            >
              <LightSpeed className="absolute inset-0 w-full h-full" />
            </ErrorBoundary>
            <button
              onClick={(e) => {
                e.stopPropagation();
            ...
            <div className="fixed bottom-12 left-1/2 -translate-x-1/2 text-white/50 text-[10px] uppercase tracking-widest font-medium z-[100000]">
        ...
        {showWarp && (
          <motion.div
            initial={{ opacity: 1, scale: 1.35, filter: "blur(16px)" }}
            animate={{ opacity: 0, scale: 1, filter: "blur(0px)" }}
            transition={{ duration: 0.9, ease: [0.4, 0, 0.2, 1] }}
            className="fixed inset-0 z-[9998] bg-gradient-to-b from-background via-muted to-background pointer-events-none"
          />
        )}
          ...
          >
            <SyntheticHero
              ...
              microDetails={copyMode === "plain"
                ...
                : ["Design-driven Engineering", "1,500+ Global Members Led", "Top 1/7000 Academic Rank"]}
            />
          </ErrorBoundary>
        </div>
        ...
        >
          <motion.div
            ...
            className="max-w-7xl mx-auto backdrop-blur-xl bg-white/40 dark:bg-white/[0.03] border border-black/5 dark:border-white/[0.12] rounded-[32px] p-8 md:p-12 shadow-2xl"
          >
            ...
            <motion.div
              ...
              className="rounded-2xl overflow-hidden border border-black/5 dark:border-white/[0.05] shadow-inner"
            >
              <ErrorBoundary
                fallback={
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
                ...
                <motion.div variants={itemVariants} className="space-y-6 text-lg text-muted-foreground leading-relaxed">
                  {copyMode === "plain" ? (
                    ...
                    <>
                      ...
                      <p>
                        My academic journey is marked by excellence, having ranked{" "}
                        <span className="text-foreground font-semibold">1 out of 7,042 students</span> across all colleges.
                  ...
                  <div className="pt-8 grid grid-cols-2 gap-8">
                    <div>
                      <h4 className="text-sm font-bold uppercase tracking-widest text-blue-600 dark:text-blue-400 mb-4">
              ...
              <div className="space-y-8">
                <motion.div
                  variants={itemVariants}
                  className="p-8 rounded-[32px] bg-zinc-50 dark:bg-zinc-900/50 border border-black/5 dark:border-white/5"
                >
              ...
              <div className="space-y-6">
                <div className="relative pl-6 border-l-2 border-blue-500/30">
                  ...
                  <h4 className="font-bold">College — USLS</h4>
                  <p className="text-sm text-muted-foreground">BS Computer Science (2021-2025)</p>
                ...
                <motion.div variants={itemVariants} className="flex flex-wrap gap-3">
                  {["11x Google Certified", "3x Canva Designer", "IBM Certified", "7x Microsoft Trophies"].map((cert) => (
                    <span
                      key={cert}
                      className="px-4 py-2 rounded-full bg-white dark:bg-zinc-900 border border-black/5 dark:border-white/10 text-xs font-medium"
                    >
                      {cert}
                    </span>
                  ))}
        ...
        <section id="contact" className="w-full py-8 sm:py-12 md:py-16 relative z-10 fade-in-on-scroll overflow-x-hidden">
          <motion.div
            ...
            viewport={{ once: true, margin: "-100px" }}
            variants={containerVariants}
            className="max-w-4xl mx-auto px-4 text-center backdrop-blur-xl bg-white/40 dark:bg-white/[0.03] border border-black/5 dark:border-white/[0.12] rounded-[32px] p-12 shadow-2xl"
          >
            ...
            <motion.div variants={itemVariants} className="flex flex-wrap justify-center gap-4" suppressHydrationWarning>
              ...
              <motion.a
                ...
                className="inline-flex items-center justify-center rounded-full border border-black/5 dark:border-white/[0.12] bg-white/40 dark:bg-white/[0.03] px-8 py-4 text-foreground dark:text-white font-bold backdrop-blur-xl shadow-xl transition-all active:scale-95"
                suppressHydrationWarning
              >
                LinkedIn
              </motion.a>
        ...
        <FloatingConsultButton imageSrc="/lib/image.jpeg" />
      </div>
    </ErrorBoundary>
  );
}
