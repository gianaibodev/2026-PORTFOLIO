...
export default function Loading() {
  ...
  return (
    <div className="relative cursor-pointer" onClick={() => setShowLoader(false)}>
      <ParticleText />
      ...
      <div
        ...
        style={{ pointerEvents: 'none' }}
      >
        {isMobile ? "Tap skip to enter" : "Press Escape or Enter to skip"}
      </div>
      {/* Desktop Space hint */}
      {!isMobile && !isHeld && (
        <div
          className="fixed top-8 left-1/2 -translate-x-1/2 z-[99999] px-4 py-2 rounded-full bg-white/5 backdrop-blur-xl border border-white/10 text-white/70 text-xs font-medium"
      ...
      {isHeld && (
        <div
          className="fixed top-8 left-1/2 -translate-x-1/2 z-[99999] px-4 py-2 rounded-full bg-white/10 backdrop-blur-xl border border-white/20 text-white text-sm font-medium animate-pulse"
          style={{ pointerEvents: 'auto' }}
        >
